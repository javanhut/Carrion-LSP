...
func (m *mockConn) Reply(ctx context.Context, id jsonrpc2.ID, result interface{}) error {
	m.replies = append(m.replies, result)
	return nil
}

func (m *mockConn) ReplyWithError(ctx context.Context, id jsonrpc2.ID, err *jsonrpc2.Error) error {
...
func newMockRequest(method string, params interface{}) *jsonrpc2.Request {
...
func TestHandler_Initialize(t *testing.T) {
	...
	if len(conn.replies) != 1 {
		t.Errorf("Expected 1 reply, got %d", len(conn.replies))
	}
	
	// Check that result is InitializeResult
	if result, ok := conn.replies[0].(protocol.InitializeResult); ok {
